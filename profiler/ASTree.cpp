...
void AST::mainHeader(const std::vector<std::string>& profileName,
  ...
  std::list<AST*>::iterator i = child.begin();
  while((*i)->tag != "function") ++i;
  
  const std::string& add = "#include \"profile.hpp\" \n";

  const std::string& declare = "profile ";
  ...
  const std::string& declare2 = "(\"\ " + temp +  "\"\) \n";
  const std::string& concat = add + declare + temp + declare2 +  space;
  
  child.insert(i, new AST(token, concat)); 
...
void AST::fileHeader(const std::string& profileName) {
  ...
  const std::string& declare = "extern profile ";
  const std::string& space = "; \n";
  std::string temp;


  const std::string& concat = add + declare + profileName + space;
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    std::cout << "entering main report" << std::endl;
    //TODO: IMPLEMENT
  ...
  do {
    ...
    AST* funcName = (*i)->getChild(name);
    mainFunc = funcName->getName();
    
  } while (mainFunc != "main");
  
    // -Get the "block" node of this function
  std::cout << "finding block node" << std::endl;

  AST* findBlock = (*i)->getChild("block");
  if ((*findBlock).tag == "block") std::cout << "at block node" << std::endl;
  
    // -Set an iterator to the child.end() of "block"
  std::list<AST*>::iterator blockEnd = (*findBlock).child.end();
  std::cout << "iterator is set" << std::endl;
  
    // -Decrement it once (to skip the "}")
  --blockEnd;
  std::cout << "blockEnd succesfully decremented" << std::endl;
    // -Search backwards for a "return" tag
  // while((*blockEnd)->tag != "return") --blockEnd;
  if ((*blockEnd)->tag == "return") std::cout << "at return tag" << std::endl;
    // -Insert the report statements before the return
  for(size_t i = 0; i < profileName.size(); ++i){
    child.insert(blockEnd, new AST(token, profileName[i]));
  }
  std::cout << "main report completed" << std::endl;
}
...
void AST::functionCount(const std::string& profileName) {
  /*  std::cout << "entering function count" << std::endl;
    //TODO: IMPLEMENT
  ...
  for(std::list<AST*>::iterator i = child.begin(); i != child.end(); ++i){
    ...
    //        Find <block>, then find <block_content>
      std::list<AST*>::iterator block = i;
      while((*block)->tag != "block") ++block;
      std::list<AST*>::iterator content = block;
      while((*content)->tag != "block_content") ++content;
    //        Insert the count as first child in <block_content>
      std::string shortName;
      for(int index = 0; index < name.size(); ++index){
	if(name[index] == '_') break;
	else
	  shortName += name[index];
      }
      const std::string& count = name + ".count(_LINE_, \"" + shortName + "\");";
      child.insert(content, new AST(token, count));
      }
    //        Example: main1_cpp.count(__LINE__, "main");
    }
    std::cout << "exiting function count" << std::endl;*/
}
...
void AST::lineCount(const std::string& profileName) {
  ...
  for(std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i){
    if((*i)->nodeType == category){
      if((*i)->tag == "expr_stmt"){
	++i;
	child.insert(i, new AST(category, count));
      }
      else {
	std::string tag = (*i)->tag;
	if(!isStopTag(tag)){
	  lineCount(profileName);
	}
      }
    }
  }
